...
Engine = Callable[[Path], InBodyPayload]
...
def extract_inbody(image_path: Path, engine: Engine = vlm.extract) -> InBodyPayload:
    ...
    the VLM baseline by default, or the fine-tuned Donut engine
    (`cera.engines.donut.load_engine(checkpoint)`). Never fabricates a number:
    the engine fails closed on unreadable fields or non-InBody input, and this
    seam applies the sheet's own redundancy as a cross-check gate regardless of
    which engine is behind it (ADR-0003, ADR-0008).
    """
    payload = engine(image_path)
