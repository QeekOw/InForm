# Strive Repo Scaffolding Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Scaffold the Strive Python backend — package skeleton, Pydantic JSON contracts, stubbed module interfaces, domain docs, and tooling — so each pipeline module can later be implemented in isolation.

**Architecture:** Backend-only Python `src/` layout. `src/strive/` holds one subpackage per pipeline stage (`ocr`, `nutrition`, `recommender`, `synthesis`) plus a central `schemas/` package that defines the JSON contracts between stages and a top-level `pipeline.py` orchestrator. Every module entry function has a typed signature and a docstring describing the paper's intended logic, but raises `NotImplementedError`. The Pydantic schemas are the real deliverable — they lock the inter-module boundaries.
...
**Spec:** `docs/superpowers/specs/2026-08-09-strive-repo-scaffolding-design.md`

---

## File Structure

Created by this plan (all paths relative to repo root `C:\Users\sukse\Strive`):
...
| `src/strive/__init__.py` | Package root, `__version__`, module overview |
| `src/strive/schemas/__init__.py` | Re-exports all contracts |
| `src/strive/schemas/user.py` | `UserProfile` (onboarding form) |
| `src/strive/schemas/inbody.py` | `InBodyPayload`, `SegmentalLean` (Module 1 output) |
| `src/strive/schemas/nutrition.py` | `NutritionTargets` (Module 2 output) |
| `src/strive/schemas/exercise.py` | `Exercise`, `ExercisePlan`, `MovementType` (Module 3 output) |
| `src/strive/schemas/master.py` | `MasterPayload` (Module 4 input), `DailyPlan` (final output) |
| `src/strive/ocr/__init__.py` + `extract.py` | Module 1 stub `extract_inbody` |
| `src/strive/nutrition/__init__.py` + `engine.py` + `constants.py` | Module 2 stub `compute_targets` + Katch-McArdle constants |
| `src/strive/recommender/__init__.py` + `filter.py` + `constants.py` | Module 3 stub `recommend_exercises` + asymmetry threshold |
| `src/strive/synthesis/__init__.py` + `generate.py` + `validate.py` | Module 4 stubs `synthesize_plan`, `validate_no_mutation` |
| `src/strive/pipeline.py` | Orchestrator stub `run_pipeline` |
...
Expected: installs pydantic, openai, pytest, ruff and `strive` (editable) with no error. (Does NOT pull torch — that is the separate `ocr` extra.)
...
**Files:**
- Create: `src/strive/__init__.py`
- Create: `src/strive/schemas/__init__.py` (placeholder for now; filled in Task 3)
- Create: `src/strive/ocr/__init__.py`
- Create: `src/strive/nutrition/__init__.py`
- Create: `src/strive/recommender/__init__.py`
- Create: `src/strive/synthesis/__init__.py`
- Create: `tests/__init__.py`

- [ ] **Step 1: Create `src/strive/__init__.py`**

```python
"""Strive — InBody AI Fitness Assistant.
...
Run: `python -c "import strive; print(strive.__version__)"`
Expected: prints `0.1.0`

- [ ] **Step 5: Commit**

```bash
git add src/strive tests
...
**Files:**
- Create: `src/strive/schemas/user.py`
- Create: `src/strive/schemas/inbody.py`
- Create: `src/strive/schemas/nutrition.py`
- Create: `src/strive/schemas/exercise.py`
- Create: `src/strive/schemas/master.py`
- Modify: `src/strive/schemas/__init__.py` (replace placeholder with re-exports)

- [ ] **Step 1: Create `src/strive/schemas/user.py`**
...
- [ ] **Step 2: Create `src/strive/schemas/inbody.py`**
...
- [ ] **Step 3: Create `src/strive/schemas/nutrition.py`**
...
- [ ] **Step 4: Create `src/strive/schemas/exercise.py`**
...
- [ ] **Step 5: Create `src/strive/schemas/master.py`**
...
from strive.schemas.exercise import ExercisePlan
from strive.schemas.inbody import InBodyPayload
from strive.schemas.nutrition import NutritionTargets
from strive.schemas.user import UserProfile
...
- [ ] **Step 6: Replace `src/strive/schemas/__init__.py` with re-exports**

```python
"""Pydantic contracts — the JSON boundaries between pipeline stages."""

from strive.schemas.exercise import Exercise, ExercisePlan, MovementType
from strive.schemas.inbody import InBodyPayload, SegmentalLean
from strive.schemas.master import DailyPlan, MasterPayload
from strive.schemas.nutrition import NutritionTargets
from strive.schemas.user import UserProfile
...
**Files:**
- Create: `src/strive/ocr/extract.py`
- Create: `src/strive/nutrition/constants.py`
- Create: `src/strive/nutrition/engine.py`
- Create: `src/strive/recommender/constants.py`
- Create: `src/strive/recommender/filter.py`
- Create: `src/strive/synthesis/generate.py`
- Create: `src/strive/synthesis/validate.py`
- Create: `src/strive/pipeline.py`

- [ ] **Step 1: Create `src/strive/ocr/extract.py`**
...
- [ ] **Step 2: Create `src/strive/nutrition/constants.py`**
...
- [ ] **Step 3: Create `src/strive/nutrition/engine.py`**
...
- [ ] **Step 4: Create `src/strive/recommender/constants.py`**
...
- [ ] **Step 5: Create `src/strive/recommender/filter.py`**
...
- [ ] **Step 6: Create `src/strive/synthesis/generate.py`**
...
- [ ] **Step 7: Create `src/strive/synthesis/validate.py`**
...
- [ ] **Step 8: Create `src/strive/pipeline.py`**
...
from strive.nutrition.engine import compute_targets
from strive.ocr.extract import extract_inbody
from strive.recommender.filter import recommend_exercises
from strive.schemas.master import DailyPlan, MasterPayload
from strive.schemas.user import UserProfile
from strive.synthesis.generate import synthesize_plan
...
python -c "
from pathlib import Path
import strive.ocr.extract as m1
import strive.nutrition.engine as m2
import strive.recommender.filter as m3
import strive.synthesis.generate as m4
import strive.synthesis.validate as m4v
import strive.pipeline as p
from strive.nutrition.constants import KATCH_MCARDLE_BASE, KATCH_MCARDLE_LBM_COEFF
from strive.recommender.constants import BILATERAL_ASYMMETRY_THRESHOLD
...
python -c "
import strive
import strive.pipeline
from strive.schemas import MasterPayload, DailyPlan
from strive.nutrition.constants import KATCH_MCARDLE_BASE
from strive.recommender.constants import BILATERAL_ASYMMETRY_THRESHOLD
print('import smoke OK', strive.__version__)
"
```
...
Expected: refs updated on `github.com/QeekOw/Strive`.

---

## Self-Review

**Spec coverage** (against `2026-08-09-strive-repo-scaffolding-design.md`):
